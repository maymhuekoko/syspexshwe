@extends('master')
@section('title','Bom Supplier Request Product')
@section('link','Bom Supplier Request Product')
@section('content')
<style>
.crd{
        ...
        <div class="card crd" style="border-radius:0px 0px 50px 50px;border-bottom: 2px solid cyan;">
            <div class="card-header">
                <div class="row justify-content-between">
                    <h4 class="text-success"><i>{{$bom_supplier->request_no}}</i><span class="text-secondary">'s Request Update Form</span></h4>
                </div>                
            </div>
            <form action="{{route('update_bom_supplier')}}" method="post" id="update_request">
                @csrf
                <input type="hidden" value="{{$bom_supplier->id}}" name="bom_supplier_id">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="request_no" name="request_no" placeholder="Enter Request No" style="border-color:cyan" value="{{$bom_supplier->request_no}}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="recipient-name" class="col-form-label text-info">Supplier</label>
                            <select class="form-control" name="supplier_id" id="supplier_id" onchange="filldetail(this.value,'{{$bom_supplier->bom_id}}')" style="border-color:cyan">
                            <option selected="selected" value="{{$bom_supplier->supplier_id}}">{{$bom_supplier->supplier->company_name}}</option>
                            @foreach($bom_supplier_arr as $sup)
                            @if($sup->id != $bom_supplier->supplier_id)
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="supplier_email" name="supplier_email" placeholder="Supplier Email" readonly style="border-color:cyan" value="{{$bom_supplier->supplier->email}}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="name" class="text-info">Title</label>
                            <input type="text" class="form-control" id="title" name="title" placeholder="Enter Title" style="border-color:cyan" value="{{$bom_supplier->rfq_email_title}}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="name" class="text-info">Tendative Reply Date</label>
                            <input type="date" class="form-control" id="reply_date" name="reply_date" placeholder="Enter Tendative Reply Date" value="{{$bom_supplier->request_quotation_date}}" style="border-color:cyan">
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="container">
                            <label class="offset-md-5 text-info">Description</label><br>
                            <textarea class="ml-3" rows="5px" cols="155px" name="description" id="desc" style="border-color:cyan">{{$bom_supplier->rfq_email_description}}</textarea>
                        </div>
                    </div>
                </div><hr>
                <div class="form-group mt-3">                   
                    ...
                    <span class="offset-md-5 mb-2">
                        ...
                        <table class="table table-bordered">
                            ...
                            <?php $i = 1;?>
                            @foreach($bom_supplier_product as $bpro)
                            <input type="hidden" id="product_id" name="product_id[]" value="{{$bpro->product_id}}">
                                <tr id="change_color{{$bpro->id}}">
                                    ...
                                    <td class="pr-3"><span id="editQty{{$bpro->id}}"><input type="text" class="form-control" id="qty{{$bpro->id}}" name="qty[]" size="1"  style="border-color:cyan" value="{{$bpro->requested_qty}}"></span></td>
                                    <td class="pr-3"><span id="editPrice{{$bpro->id}}"><input type="text" class="form-control" id="price{{$bpro->id}}" name="price[]" size="1"  style="border-color:cyan" value="{{$bpro->requested_price}}"></span></td>
                                    <td class="pr-3"><span id="editSpec{{$bpro->id}}"><input type="text" class="form-control" id="spec{{$bpro->id}}" name="spec[]" size="1" placeholder="specs..." style="border-color:cyan" value="{{$bpro->requested_specs}}"></span></td>
            ...
            <div class="card-footer">
                        ...
                        <button type="button" class="btn btn-warning" style="margin-left:580px" onclick="sendcheck()"><i class="fas fa-save mr-2"></i>Update</button>
                   
                   
                   
                        
                            
                            @if($bom_supplier->email_sent_status == 0)
                            ...
                            @else
                            <button type="button" class="btn btn-secondary" style="margin-right:200px" disabled><i class="fas fa-envelope mr-2"></i>Sent Email</button>
...
function sendcheck()
{
    var bomproduct = @json($bom_supplier_product);
