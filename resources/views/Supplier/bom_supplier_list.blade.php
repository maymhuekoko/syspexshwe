            ...
            <div class="card-body">
                <table class="table table-bordered table-striped">
                    ...
                    <tbody class="text-center">
                        ...
                        @if($bayofmat->id == $bsup->bom_id)
                        <tr>
                            <!-- <input type="hidden" id="bomsup_id" value="{{$bsup->id}}"> -->
                            <td>{{$i++}}</td>
                            <td>{{$bsup->request_no}}</td>
                            <td>{{$bsup->supplier->company_name}}</td>
                            @if($bsup->request_quotation_date == null)
                            <td><span class="text-danger"><b><span class="badge badge-danger">Not Yet</span></b></span></td>
                            @else
                            ...
                            <td><span class="text-danger"><b><span class="badge badge-danger">Not Yet</span></b></span></td>
                            @else
                            <td class="text-success"><b>{{$bsup->quotation_reply_date}}</b></td>
                            @endif
                            @if($bsup->po_sent_date == null)
                            <td><span class="text-danger"><b><span class="badge badge-danger">Not Yet</span></b></span></td>
                            @else
                            <td class="text-success"><b>{{$bsup->po_sent_date}}</b></td>
                            @endif
                            @if($bsup->invoice_received_date == null)
                            <td><span class="text-danger"><b><span class="badge badge-danger">Not Yet</span></b></span></td>
                            @else
                            <td class="text-success"><b>{{$bsup->invoice_received_date}}</b></td>
                            @endif
                            ...
                            <td>
                                <div class="form-group">
                                    ...
                                    <select class="form-control bg-info border border-info" name="attach_file" style="border-radius:15px" id="attach_file_{{$bsup->id}}" onchange="attach_option(this.value,'{{$bsup->id}}','{{$bsup->request_no}}')">
                                    ...
                                    <option value="5" class="bg-light">ByPass Input(GRN)</option>
                                    </select>
                                </div>
                            </td>
                            <td>
                                ...
                                <a href="{{route('update_bom_supplier_req',$bsup->id)}}" class="btn btn-warning">Update</a>
                ...
                <form action="{{route('show_supplier_purchase_order')}}" method="get" id="po">
                    @csrf
                ...
                <form action="{{route('show_grn_form')}}" method="get" id="show_grn">
                    @csrf
                    <input type="hidden" name="bom_supplier_id" id="grn_bom_supplier_id">
                </form>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" id="quodate{{$bsup->id}}" name="quodate" value="<?php echo date('Y-m-d'); ?>">
                            ...
                            @else
                            <button type="button" id="view_quo_file_{{$bsup->id}}" class="btn btn-warning btn-block" data-toggle="modal" data-target="#quo_file{{$bsup->id}}">View Attach File</button>
                            @endif
                            </div>
                        </form>
                        </div>
                ...
                <!-- Begin show quotation file -->
                <div class="modal fade bd-example-modal-lg" id="quo_file{{$bsup->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header bg-info">
                            <h5>Quotation File Viewer</h5>
                        </div>
                        <dic class="card">
                            <div class="card-body" id="">
                                <!-- <span id="show_quotation_file{{$bsup->id}}"></span> -->
                                <img src="/image/1r.jpg" class="img-fluid text-center attachmentimg" style="width: -webkit-fill-available;height: 100vh;">
                            </div>
                        </div>
                    </div>
                </div>
                </div>
                <!-- End show quotation file -->
                @endforeach
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" id="invdate{{$bsup->id}}" name="invdate" value="<?php echo date('Y-m-d'); ?>">
...
$( document ).ready(function() {
    ...
    $.each( quot, function( i, val ) {
        ...
        `;
        // htmlfile = `
        // ${quot[i].quotation_file_path}
        //         <iframe src ="image/send_form.jpg" class="img-fluid text-center attachmentimg" style="width: -webkit-fill-available;height: 100vh;"></iframe>
        //         `;
        // $('#show_quotation_file'+quot[i].bom_supplier_id).html(htmlfile);
...
function attach_option(value,bomsup_id,req_no)
{
    ...
    {
        alert(bomsup_id);
        $('#grn_bom_supplier_id').val(bomsup_id);
        $('#show_grn').submit();
    }
